...
def list_appointments(creds, dept_uid, date_str, DEBUG = False, SIMULATION = False):
    """A simple method to list all appointments for a date.

    This method will take the credentials (username, password, and base URL)
    the departmentUUID, and a string with the ISO date YYYY-MM-DD, and GET the 
    list of appointments for the date from the API. An optional DEBUG boolean 
    ...
    url = "%s/dealerDepartment/%s/date/%s" % (creds[KEY_BASE_URL],dept_uid,date_str)
    if SIMULATION:
        ...
        if DEBUG:
            print(r.json())
        return r.json()['serviceAppointments']
